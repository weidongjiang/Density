 ...
 *  @return 拼接完成的路径
 */
NSString *hy_LibraryFileWithName(NSString *name);//Library下某个文件
 ...
 *  @return Bundle路径
 */
NSString *hy_NSBundleFolder(void);//BundlePath,iOS下同资源目录
 ...
 *  @return 拼接完成的路径
 */
NSString *hy_ResourceWithName(NSString *name);//资源文件
 ...
 *  @return 拼接完成的路径
 */
NSString *hy_TempFileWithName(NSString *name);//临时目录
 ...
 *  @return 磁盘剩余空间
 */
long hy_getDiskFreeSize(void);

/*!
 *  将文件HASH化的相关工具方法。
 */
@interface NSFileManager (HYTCrypto)
/*!
 *  生成并返回文件的MD5哈希字符串
 *
 *  @param filePath 文件路径
 ...
 *  文件操作的相关操作
 */
@interface NSFileManager(HYFileManagerUtilities)
/*!
 *  判断删除对应路径文件或文件夹是否成功
 *
 ...
 *  @return 对应位置长度的文件数据
 */
+ (NSData*)hy_dataOfFile:(NSString*)filePath
 ...
 */
+ (BOOL)hy_copyItemAtPath:(NSString*)fromPath
 ...
 *  获取文件目录
 */
@interface NSFileManager(HYFileExist)
/*!
