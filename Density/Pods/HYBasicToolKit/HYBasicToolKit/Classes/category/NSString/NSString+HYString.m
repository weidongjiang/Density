...
- (NSString *)hy_interceptionByGBK:(int)length {
    ...
    for (int i = 0; self.length > i; i++) {
        ...
        if ([currentString hy_stringCount] <= length) {
...
- (NSString *)hy_urlEncode {
    return [self hy_urlEncodeUsingEncoding:NSUTF8StringEncoding];
}
...
- (NSString *)hy_urlDecode {
    return [self hy_urlDecodeUsingEncoding:NSUTF8StringEncoding];
}
...
- (NSString *)hy_clipFitStringForLabel:(CGSize)labelSize font:(UIFont *)font {
    ...
    CGFloat originalWidth = [self hy_singleLineWidthWithFont:font];
    if (originalWidth <= maxWidth) return [self copy];
    CGFloat pointStrLength = [@"" hy_singleLineWidthWithFont:font];
    ...
    for (NSInteger i = originalString.length - 1;i >= 0;i--){
        ...
        CGFloat tempStringWidth = [tempString hy_singleLineWidthWithFont:font];
